...
io.on('connection', (socket) => {
    ...
    socket.on('respond_to_friend_request', async ({ requesterId, accepted }) => {
        ...
        try {
            ...
            if (accepted) {
                ...
                socket.emit('friend_request_accepted_confirmation', { friendId: requesterId.toString(), friendUsername: requesterDoc.username }); 
                ...
                io.emit('update_user_list', getOnlineUsernames()); // Friend status might affect UI or online user interactions
                 // Send full friend data update to both users involved
                 socket.emit('request_friend_data_and_user_list'); // For the responder
                 if (requesterSocketId) io.to(requesterSocketId).emit('force_friend_data_refresh'); // Tell requester to refresh their friend data
            } else {
    ...
    socket.on('remove_friend', async ({ friendUsername }) => {
        ...
        try {
            ...
            io.emit('update_user_list', getOnlineUsernames());
             // Send full friend data update to both users involved
             socket.emit('request_friend_data_and_user_list'); // For the remover
             if (friendSocketId) io.to(friendSocketId).emit('force_friend_data_refresh'); // Tell removed friend to refresh their data
     ...
     // Combined friend data and user list request
     socket.on('request_friend_data_and_user_list', async () => {
        ...
        try {
            ...
            if (!userDoc) {
                socket.emit('update_user_list', getOnlineUsernames());
                socket.emit('update_friend_data', { friends: [], pendingSent: [], pendingReceived: [] });
                return;
            }
            const friendData = {
            ...
            console.error("SERVER LOG: Error fetching friend/user data:", socket.username, error);
            socket.emit('update_user_list', getOnlineUsernames());
            socket.emit('update_friend_data', { friends: [], pendingSent: [], pendingReceived: [] });
        }
    });
     // Client requests this to refresh its friend data after an action taken by another user
     socket.on('force_friend_data_refresh_request', async () => { // Renamed event
         console.log(`SERVER LOG: Force friend data refresh requested by ${socket.username} (${socket.id})`);
         socket.emit('request_friend_data_and_user_list'); // Trigger the combined data emission
     });
